...
Modelling tumour immune environments from copy number signature
To adequately model the interaction between tumour immune environment and chromosomal instability, measured by immunohistochemistry markers and copy number signatures, respectively. Utilising the copy number signatures as a predictor variable, we implemented a [generalised/linear mixed model] which allows for the incorporation of compositional nature of copy number signature exposures and the multiple correlated images generated per sample in the IHC data in order to assess if the tumour immune environment can be predicted from copy number signatures. We also included several clinical covariates such as tissue type, age, stage, and lines of therapy as covariates to estimate the effects of these features on the response variable.

Copy number signatures were transformed from a relative compositional state using a [binarisation/ALR/ILR/binarisation-logisticPCA], similarly to the partial ILR transformation utilised in the signature comparison model described previously, where this chosen transformation results in the best predictive performance. The model was constructed in R using the [lme4/lmerTest] package and model performance was tested between a model with transformed copy number signatures compared to a null model without including the transformed copy number signatures assessed using a [Likelihood-ratio/anova] test.
...
sig_immune_cor_t <- ggplot(sig_immune[sig_immune$tissue == "tumour",]) +
    ...
    stat_cor(aes(log1p(normalised_cells),exposure,label = ..r.label..),label.sep = "\n",
             r.digits = 2,
             size = 2.5,
             label.x.npc = 0.4,
             label.y.npc = 0.9,
