...
plot_sigs_immune <- function(data=NULL,data2=NULL,marker=NULL){
    ...
    sig_bar.a$SAMPLE_ID <- factor(sig_bar.a$SAMPLE_ID,levels = unique(immune.data.a$ID[order(immune.data.a$normalised_cells)]))
...
im_list <- c(1,2,32,43,48,54,72,86,111,125,135,144,147,177,186,201,219,229,234,244,261)
```
...
immune.data.wide <- immune.data %>%
                      pivot_wider(id_cols = c("ID","tissue"),names_from = marker,values_from = normalised_cells)

pairs_plot <- GGally::ggpairs(immune.data.wide,aes(color=tissue),columns = 3:6) +
  theme_bw()
...
saveRDS(object = sig_immune_cor_a,file = "plots/sig_immune_cor_all.RDS")
ggsave(sig_immune_cor_a,filename = "plots/sig_immune_cor_all.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(sig_immune_cor_a,filename = "plots/sig_immune_cor_all.pdf",width = 8,height = 6,units = "in",dpi = 300)
sig_immune_cor_a

saveRDS(object = sig_immune_cor_t,file = "plots/sig_immune_cor_tumour.RDS")
ggsave(sig_immune_cor_t,filename = "plots/sig_immune_cor_tumour.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(sig_immune_cor_t,filename = "plots/sig_immune_cor_tumour.pdf",width = 8,height = 6,units = "in",dpi = 300)
sig_immune_cor_t

saveRDS(object = sig_immune_cor_s,file = "plots/sig_immune_cor_stroma.RDS")
ggsave(sig_immune_cor_s,filename = "plots/sig_immune_cor_stroma.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(sig_immune_cor_s,filename = "plots/sig_immune_cor_stroma.pdf",width = 8,height = 6,units = "in",dpi = 300)
sig_immune_cor_s
```

```{r glmm}
length(unique(sig_immune$patient_id))
# library(lme4)
# glmm <- glmer(exposure ~ log(normalised_cells) + tissue  + (1|patient_id),family = poisson(),
#              data = sig_immune[sig_immune$marker == "CD8" & sig_immune$tissue != "all" & sig_immune$signature == "s1",])
# summary(glmm)
# anova(glmm)
```

```{r combine_samples}
sig_immune_comb_samples <- sig_immune %>%
                            group_by(patient_id,marker,signature,tissue) %>%
                            summarise(across(.cols = c(normalised_cells,exposure),.fns = median))
dim(sig_immune)
dim(sig_immune_comb_samples)

sig_immune_cor_a2 <- ggplot(sig_immune_comb_samples[sig_immune_comb_samples$tissue == "all",]) +
    geom_point(aes(log(normalised_cells),exposure,color=signature)) +
    geom_smooth(aes(log(normalised_cells),exposure),method = "lm",se=F) +
    facet_grid(rows = vars(marker),cols = vars(signature)) +
    stat_cor(aes(log(normalised_cells),exposure),label.sep = "\n",
...
sig_immune_cor_t2 <- ggplot(sig_immune_comb_samples[sig_immune_comb_samples$tissue == "tumour",]) +
    geom_point(aes(log(normalised_cells),exposure,color=signature)) +
    geom_smooth(aes(log(normalised_cells),exposure),method = "lm",se=F) +
    facet_grid(rows = vars(marker),cols = vars(signature)) +
    stat_cor(aes(log(normalised_cells),exposure),label.sep = "\n",
...
sig_immune_cor_s2 <- ggplot(sig_immune_comb_samples[sig_immune_comb_samples$tissue == "stroma",]) +
    geom_point(aes(log(normalised_cells),exposure,color=signature)) +
    geom_smooth(aes(log(normalised_cells),exposure),method = "lm",se=F) +
    facet_grid(rows = vars(marker),cols = vars(signature)) +
    stat_cor(aes(log(normalised_cells),exposure),label.sep = "\n",
