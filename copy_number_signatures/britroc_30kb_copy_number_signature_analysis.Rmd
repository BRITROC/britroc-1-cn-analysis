...
sig_box_plot <- function(sig_quants = NULL,test="t.test",plot_title = NULL,paired=FALSE){
  ...
  x <- sig_quants
  if(paired){
    sig_box_data <- merge(x = rownames_to_column(
  ...
  ggplot(data = sig_box_data_long) +
    #geom_jitter(aes(x = Signature,y = value,color=group),alpha=0.4) +
...
ggplot(samp_comp_long,aes(x = value)) +
  ...
  # geom_signif(aes(x = name,y=value,group=name),
...
#sig_bar_plot_paired_alt1

# ggplot(sig_bar_paired) +
#   geom_col(aes(PATIENT_ID,value,fill=signature,group=group),position = "dodge") +
...
# side by side bar plot is not easily implemented in ggplot when including relative bars, stacked bars, and position dodging

#sig_bar_plot_paired_alt2

sig_bar_paired_alt <- sig_bar_paired %>%
                        mutate(value = ifelse(group == "arx",-value,value))

#sig_bar_paired_alt$signature <- factor(sig_bar_paired_alt$signature,levels = rev(unique(sig_bar_paired_alt$signature)))
...
sig_bar_plot_paired_alt <- ggplot(sig_bar_paired_alt) +
  ...
  annotate(geom = "text",x = 48.3,y = c(-0.85,0.88),label = c("diagnosis","relapse")) +
...
```{r sig_box_plot}
sig_box_plot(sig_quants = sig_quants,
             plot_title = "Signature exposures by signature between tumour groups (all)",
             test = "t.test")
...
```{r sig_box_plot_pair}
sig_box_plot(sig_quants = sig_quants[,colnames(sig_quants) %in% patient.meta$SAMPLE_ID[patient.meta$paired == TRUE]],
             test = "t.test",paired = TRUE,
             plot_title = "Signature exposures by signature between tumour groups (paired)")
...
ggplot(data = x) +
    geom_boxplot(aes(x = Signature,y = value,fill=group)) +
    # geom_signif(aes(x = Signature,y = value),
    #             annotations = ifelse(test.p < 0.01,formatC(test.p,format = "e",digits = 2),signif(test.p,digits = 3)),
    #                                  xmin = seq.int(0.5,6.5,1)+0.2,
    #                                  xmax = seq.int(1.5,7.5,1)-0.2,
    #                                  y_position = 0.9) +
    labs(title = "Signature exposures by signature between cohorts",caption = paste("p-values calculated using ","test",sep = "")) +
    scale_y_continuous(name = "Exposure",limits = c(0,1)) +
    theme_bw()
...
ggplot(subset_sig_quants) + geom_point(aes(x = value,y = natgen_value)) +
