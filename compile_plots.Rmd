...
flow_chart <- ggdraw() + draw_image(magick::image_read_pdf("plots/BriTROC-1_manuscript2_figure_1_v2-cropped.pdf",density = 600))
...
sig_primary_radar <- sig_primary_radar + theme(legend.position = "none")

## Modify plots
sig_bar_plot_primary_pt_diag <- sig_bar_plot_primary_pt_diag + theme(legend.position = "none")
sig_dist_primary_pt_diag <- sig_dist_primary_pt_diag + theme(legend.position = "none",axis.text.x = element_blank())
...
```{r fig_pt_status}
## Load plots
## figure legend Figure XX. Copy number signatures stratified by platinum-based treatment sensitivity 
# A - Copy number signature spectrum of copy number profiles from diagnosis and relapse HGSC tumours across all samples, stratified by response to platinum-based chemotherapeutics (n = 30, n = 94, n = 36, and n = 101, diagnosis resistant, diagnosis sensitive, relapse resistant, and relapse sensitive, respectively). B - Boxplot demonstrating copy number signature distributions in unpaired copy number between resistant and sensitive response to platinum-based chemotherapeutics. C - Boxplot demonstrating copy number signature distributions in unpaired copy number between resistant and sensitive response to platinum-based chemotherapeutics, stratified by diagnosis and relapse tumours.
...
## Figure s## compile
fig7S <- plot_grid(plot_grid(sig_pt_bar_plot,sig_pt_plot,sig_pt_plot_strat_group,
                            labels = c("A","B","C"),
                            nrow = 3,
...
## Plot render
ggsave2(filename = "plots/figure_7S_render.png",plot = fig7S,width = 8,height = 12,units = "in",dpi = 300)
ggsave2(filename = "plots/figure_7S_render.pdf",plot = fig7S,width = 8,height = 12,units = "in",dpi = 300)
fig7S
```
...
cn_tissue_counts_strat <- readRDS("copy_number_analysis/sites_of_relapse/plots/cna_count_tissue.RDS")
cn_tissue_counts_strat <- cn_tissue_counts_strat + theme(legend.position = "none",axis.title.x = element_blank(),
                                                         axis.ticks.x = element_blank(),
                                                         axis.text.x = element_blank())
cn_tissue_sigs_strat <- readRDS("copy_number_analysis/sites_of_relapse/plots/signature_tissue.RDS")
...
plot.list <- lapply(pdf.list,FUN = function(x){
  name <- gsub(gsub(x = x,pattern = "copy_number_signatures/plots/ternary_all_combinations/ternary_all_",replacement = ""),pattern = ".pdf",replacement = "")
  plot <- ggdraw() +
...
ternary_all <- plot_grid(plot_grid(plotlist = plot.list,nrow = 7,ncol = 5),
                sig_legend_b,nrow = 2,rel_heights = c(0.9,0.1))
