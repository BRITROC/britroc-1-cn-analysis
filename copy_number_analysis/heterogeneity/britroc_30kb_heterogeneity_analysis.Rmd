...
CORRECTION - No smoothing is now applied to either the upper or lower bounds applied to each segment to more closely conform with the approach seen by van Dijk et al.see [here](https://www.nature.com/articles/s41467-021-23384-6).

```{r segment_noise}
samples <- colnames(abs_data)[1:10]
segment_noise <- do.call(rbind,lapply(samples,FUN = function(x){
  ...
  tsd <- t %>%
    ...
    group_by(segment) %>%
    mutate(bin_count = n()) %>%
    mutate(seg_sd = sd(bin)) %>%
    mutate(seg_start = min(start)) %>%
    mutate(seg_end = max(end)) %>%
    dplyr::select(-change,-bin,-start,-end,-changepoint) %>%
    distinct() %>%
    mutate(sample_mean = weighted.mean(seg_sd,w = bin_count))
    
  tsd$sample <- rep(sample,times=nrow(tsd))
  return(tsd)
}))
    
ggplot(segment_noise) + geom_density(aes(seg_sd))
```
...
The clonality value was extracted from each sample utilising the parameters defined previously.

```{r set_clonality}
clonality_table <- add_clonality_meta(get_clonality(abs_data = abs_data,
                             weighted = TRUE,
                             smooth = FALSE,
                             wiggle = 0.1)) %>%
