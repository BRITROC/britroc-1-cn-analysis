...
Cytoband data was pulled from USCS table schema for hg19. Acro-centromeric and stalk bands were dropped.
...
focal_genes <- read.table("../focal_analysis/data/focal_CNA_pseudo_bed.txt",header = T,sep = "\t") %>%
                dplyr::rename(chr = "Chromosome.scaffold.name",start="Gene.start..bp.",end="Gene.end..bp.",gene = "Gene.name") %>%
                dplyr::select(chr,start,end,gene,expected)
positions <- paste0(focal_genes$chr,":",focal_genes$start,"-",focal_genes$end)
```

```{r biomart}
if(file.exists("../focal_analysis/data/protein_coding_genes.tsv")){
	protein_coding_genes <- read.table("../focal_analysis/data/protein_coding_genes.tsv",header=T,sep="\t")
} else {
	...
	write.table(protein_coding_genes,"../focal_analysis/data/protein_coding_genes.tsv",col.names=T,row.names=F,sep="\t")
}
```

```{r load_NCG}
ncg <- read.table("data/NCG_cancerdrivers_annotation_supporting_evidence.tsv",header = T,sep = "\t")
```   
...
ggsave("plots/cna_freq_plot.pdf",plot = genome_plot,device = "pdf",width = 12,height = 6,units = "in",dpi = 300)
genome_plot
```

```{r source_data_s4}
write.table(x = bin_cna[bin_cna$cna != "neutral",],
            file = "../../source_data/figure_S4.tsv",
...
## Test adjusted per chromosome
paired_diffs_stats$wilcox_adj <- unlist(lapply(split(paired_diffs_stats,f = paired_diffs_stats$chr),
                                               FUN = function(x){p.adjust(x$wilcox_pval,method = "fdr")}),use.names = F)

paired_diffs_stats$wilcox_sig <- ifelse(paired_diffs_stats$wilcox_adj < 0.05,"significant","non-significant")
```
...
ggsave(filename = "plots/genome_paired_substractionPlot.png",
       ...
       units = "in",
       dpi = 300)
ggsave(filename = "plots/genome_paired_substractionPlot.pdf",
       plot = p,
       width = 12,
       height = 6,
       units = "in",
...
```{r source_data_fig3A}
write.table(x = paired_diffs_stats,
            file = "../../source_data/figure_3A_layer1.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)

write.table(x = chrlabs,
            file = "../../source_data/figure_3A_layer2.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
cat("Signif bins overlapping protein-coding genes: ",length(proteinOverlaps),"\n")
cat("Any in relevant gene list:",any(focal_genes$gene %in% proteinOverlaps),"\n")
writeLines(text = proteinOverlaps,con = "data/substraction_protein_overlaps.txt",sep = "\n")
ncg_results <- ncg[ncg$symbol %in% proteinOverlaps,]
cat("NCG matching genes: ",length(unique(ncg_results$symbol)),"\n")
write.table(x = ncg_results,file = "data/ncg_analysis_results.tsv",quote = F,sep = "\t",row.names = F,col.names = T)
```
...
GO.ids <- mapIds(x = org.Hs.eg.db,keys = as.character(protein_coding_genes$hgnc_symbol),keytype = "SYMBOL",column = "GO",multiVals = "list")
geneL <- rep(1,times=length(protein_coding_genes$hgnc_symbol))
names(geneL) <- as.character(protein_coding_genes$hgnc_symbol)
geneL[names(geneL) %in% proteinOverlaps] <- 0
...
write.table(x = allRes[allRes$qVal < 0.05,],file = "data/substraction_topGO_results.tsv",quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r reactome_pathway}
entrez_ids <- mapIds(x = org.Hs.eg.db,keys = as.character(proteinOverlaps),keytype = "SYMBOL",column = "ENTREZID")
reactome <- enrichPathway(gene=entrez_ids,pvalueCutoff=0.05, readable=T)
reactome_results <- reactome@result
head(reactome_results)
write.table(x = reactome_results[reactome_results$pvalue < 0.05,],file = "data/substraction_reactome_results.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
paired_segs <- data.frame(nsegs=nsegs) %>%
  ...
  summarise(segs=median(nsegs)) %>%
  pivot_wider(id_cols = "PATIENT_ID",names_from = "group",values_from = "segs") %>%
  dplyr::rename(diagnosis="arx",relapse="rlps")
...
seg_paired_long <- pivot_longer(data = paired_segs,cols = 2:3,
                                names_to = "group",
                                values_to = "segments") %>%
...
segs_paired <- ggplot(seg_paired_long) +
  geom_jitter(aes(group,segments)) +
...
ggsave(plot = segs_paired,filename = "plots/seg_counts_paired.pdf",width = 5,height = 7,units = "in",dpi = 300)
```

```{r source_data_S7A}
write.table(x = seg_paired_long,
            file = "../../source_data/figure_S7A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave(plot = segs_paired_pt,filename = "plots/seg_counts_pt_paired.pdf",width = 5,height = 7,units = "in",dpi = 300)
```

```{r source_data_S7C}
write.table(x = seg_paired_long,
            file = "../../source_data/figure_S7C.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
seg_Corr <- ggplot(paired_segs) +
        ...
        xlab("diagnosis tumour") +
        ylab("relapse tumour") +
        theme_bw() + annotate(geom = "text",x = 375,y=10,label=paste0("kendall tau:",
                                                                      round(seg.kenall$estimate,digits = 3)))
seg_Corr
```

```{r source_data_S7B}
write.table(x = paired_segs,
            file = "../../source_data/figure_S7B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave2(plot = seg_plot,filename = "plots/segments_combined_plot.pdf",width = 6,height = 7,units = "in",dpi = 300)
```
...
ggsave(plot = ploidy_plot,filename = "plots/ploidy_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_plot,filename = "plots/ploidy_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_pt_plot,filename = "plots/ploidy_pt_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_pt_plot,filename = "plots/ploidy_pt_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
ploidy_plot
ploidy_pt_plot
```

```{r source_data_S6A}
write.table(x = ploidy_summary,
            file = "../../source_data/figure_S6A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r source_data_S6C}
write.table(x = ploidy_summary[!is.na(ploidy_summary$platinum_status),],
            file = "../../source_data/figure_S6C.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave(plot = ploidy_paired_plot,filename = "plots/ploidy_paired_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_paired_plot,filename = "plots/ploidy_paired_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_paired_pt_plot,filename = "plots/ploidy_paired_pt_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = ploidy_paired_pt_plot,filename = "plots/ploidy_paired_pt_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
...
```{r source_data_S6B}
write.table(x = ploidy_summary_paired,
            file = "../../source_data/figure_S6B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r source_data_S6D}
write.table(x = ploidy_summary_paired,
            file = "../../source_data/figure_S6D.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave2(filename = "plots/ploidy_plots.pdf",plot = ploidy_plots,width = 8,height = 8,units = "in",dpi = 300)
ploidy_plots
```

```{r ploidy_changes}
labels <- c("diploid-triploid","diploid-triploid",
            "diploid-tetraploid","diploid-tetraploid",
            "diploid-tetraploid","diploid-tetraploid",
            "diploid-triploid","diploid-triploid",
            "pentaploid-triploid","pentaploid-triploid",
            "diploid-tetraploid","diploid-tetraploid",
            "diploid-triploid","diploid-triploid",
...
ploidy_change_table <- ploidy_summary_paired[ploidy_summary_paired$ploidy_change == TRUE,] %>%
                        ungroup() %>%
                        dplyr::select(-paired,-ploidy_change) %>%
                        mutate(direction = ifelse(ploidy_diff < 0,"decrease","increase")) %>%
                        mutate(ploidy_change = labels)

ploidy_change_table

lapply(split(ploidy_change_table[ploidy_change_table$direction == "increase",],
             f = list(ploidy_change_table$group[ploidy_change_table$direction == "increase"])),FUN = function(x) summary(x$ploidy))

table(ploidy_change_table$ploidy_change[ploidy_change_table$group == "relapse"])

write.table(x = ploidy_change_table,
            file = "data/ploidy_change_pats.tsv",
            quote = F,row.names = F,col.names = T,sep = "\t")
```

```{r test_pt_ploidy_change}
cat("pt_sens_ploidy_change ","pval=",fisher.test(matrix(ncol = 2,
                   rbind(c(table(clinical.data$pt_sensitivity_at_reg[!paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)])),
                         c(table(clinical.data$pt_sensitivity_at_reg[paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)])))))$p.value,"\n",sep = "")

cat("age_ploidy_change ","pval=",wilcox.test(x = clinical.data$age[!paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)],
            y = clinical.data$age[paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)])$p.value,"\n",sep="")

cat("prior_chem_ploidy_change ","pval=",wilcox.test(x = clinical.data$pre_reg_chemo[!paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)],
            y = clinical.data$pre_reg_chemo[paste0("BRITROC-",clinical.data$britroc_number) %in% unique(ploidy_change_table$PATIENT_ID)])$p.value,"\n",sep="")
```
...
ggsave(plot = purity_plot,filename = "plots/purity_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_plot,filename = "plots/purity_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_pt_plot,filename = "plots/purity_pt_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_pt_plot,filename = "plots/purity_pt_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
...
```{r source_data_S5A}
write.table(x = purity_summary,
            file = "../../source_data/figure_S5A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r source_data_S5C}
write.table(x = purity_summary[!is.na(purity_summary$platinum_status),],
            file = "../../source_data/figure_S5C.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave(plot = purity_paired_plot,filename = "plots/purity_paired_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_paired_plot,filename = "plots/purity_paired_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_paired_pt_plot,filename = "plots/purity_paired_pt_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = purity_paired_pt_plot,filename = "plots/purity_paired_pt_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
...
```{r source_data_S5B}
write.table(x = purity_summary_paired,
            file = "../../source_data/figure_S5B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r source_data_S5D}
write.table(x = purity_summary_paired,
            file = "../../source_data/figure_S5D.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave2(filename = "plots/purity_plots.pdf",plot = purity_plots,width = 8,height = 8,units = "in",dpi = 300)
purity_plots
```
...
CNA_events$platinum_treatment <- clinical.data$pt_sensitivity_at_reg[match(CNA_events$patient,
                                                                           paste0("BRITROC-",clinical.data$britroc_number))]
...
CNA_events_paired_median <- CNA_events_paired %>%
  group_by(group) %>%
  dplyr::select(-patient) %>%
  summarise_all(.funs = list(mean=mean,median=median,iqr=stats::IQR,samples=length))
CNA_events_paired_median

CNA_events_paired$platinum_treatment <- clinical.data$pt_sensitivity_at_reg[match(CNA_events_paired$patient,
                                                                                  paste0("BRITROC-",clinical.data$britroc_number))]
```
...
ggsave(filename = "plots/cnv_event_means_broad_paired.png",
       ...
       units = "in",
       dpi = 300)
ggsave(filename = "plots/cnv_event_means_broad_paired.pdf",
       plot = cemb,
       width = 6,
       height = 6,
       units = "in",
...
ggsave(filename = "plots/cnv_event_means_broad__ptsen_paired.png",
       ...
       units = "in",
       dpi = 300)
ggsave(filename = "plots/cnv_event_means_broad__ptsen_paired.pdf",
       plot = cembp,
       width = 6,
       height = 6,
       units = "in",
...
```{r source_data_S8}
write.table(x = CNA_events_paired_long[!is.na(CNA_events_paired_long$platinum_treatment),],
            file = "../../source_data/figure_S8.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
kable(cytoband_fishers_arx_rlps) %>%
  kable_styling(full_width = T) %>%
  scroll_box(height = "200px")
```
...
ggsave(plot = cyto_plot_paired,filename = "plots/cyto_plot_paired_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = cyto_plot_paired,filename = "plots/cyto_plot_paired_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
cyto_plot_paired
```

```{r source_data_S10A}
write.table(x = broad_cyto_combined,
            file = "../../source_data/figure_S10A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave(plot = arm_plot_paired,filename = "plots/arm_plot_paired_plot.png",width = 8,height = 6,units = "in",dpi = 300)
ggsave(plot = arm_plot_paired,filename = "plots/arm_plot_paired_plot.pdf",width = 8,height = 6,units = "in",dpi = 300)
arm_plot_paired
```

```{r source_data_S10B}
write.table(x = broad_arm_combined,
            file = "../../source_data/figure_S10B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave2(plot = broad_plot,filename = "plots/cna_arm_cyto_plot.pdf",width = 10,height = 8,units = "in",dpi = 300)
broad_plot
```
