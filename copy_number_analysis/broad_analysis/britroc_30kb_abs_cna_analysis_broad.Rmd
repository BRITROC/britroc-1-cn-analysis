...
ploid_change_pats <- str_sort(unique(ploidy_summary_paired$PATIENT_ID[ploidy_summary_paired$ploidy_change == TRUE]),numeric = T)
cat("Ploidy change patients: ",paste0(as.character(ploid_change_pats),collapse = ", "))
cat(length(ploid_change_pats))
```

```{r test_pt_ploidy_change}
fisher.test(matrix(ncol = 2,
                   rbind(c(table(clinical.data$pt_sensitivity_at_reg[!paste0("BRITROC-",clinical.data$britroc_number) %in% ploid_change_pats])),
                         c(table(clinical.data$pt_sensitivity_at_reg[paste0("BRITROC-",clinical.data$britroc_number) %in% ploid_change_pats])))))

wilcox.test(x = clinical.data$age[!paste0("BRITROC-",clinical.data$britroc_number) %in% ploid_change_pats],
            y = clinical.data$age[paste0("BRITROC-",clinical.data$britroc_number) %in% ploid_change_pats])
```
