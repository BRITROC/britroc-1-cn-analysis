...
plotSegments <- function(object=NULL,pat.name=NULL,cn.max=15){
    diff_matrix_s <- object[,pat.name]
    fdfiltfull <- as.data.frame(str_split(string = names(diff_matrix_s),pattern = ":|-",simplify = TRUE))
    colnames(fdfiltfull) <- c("chromosome","start","end")
    
    segTable<-c()
    for(c in unique(fdfiltfull$chromosome)){
        snfilt<-diff_matrix_s[fdfiltfull$chromosome==c]
    ...
    segTable$chromosome <- factor(segTable$chromosome,levels = stringr::str_sort(unique(segTable$chromosome),numeric = T))
    segTable <- segTable[order(segTable$chromosome,segTable$start),]
    ...
    if(max(segTab$segVal) > cn.max){
        ...
        ylim <- c(-round(max(segTab$segVal))-1,round(max(segTab$segVal))+1)
    }
    ...
    title <- pat.name
    sub.title <- paste0("segments: ",seg.n)
    ...
    graphics::abline(h = seq.int(round(min(segTab$segVal))-1,cn.max,1),lty="dashed",col="gray50")
    graphics::segments(x0 = segTab$startf,y0 = segTab$segVal,x1 = segTab$endf,y1 = segTab$segVal,lwd=3,col="blue")
}

get_patient_vig <- function(diff_matrix = NULL,pat.name = NULL){
  pat.name <<- pat.name
  seg_formula <- ~{
  plotSegments(object = diff_matrix,pat.name = pat.name)
  }
  seg_plot <- ggdraw(seg_formula)
  ...
  ith_p2 <- ggplot() +
              geom_jitter(data = ith[ith$patient != pat.name,],aes(group,ith,colour=group),
                          alpha=0.4,
                          position = position_dodge2(width = 0.4)) +
              scale_color_manual(values = colour_palettes$diagnosis_relapse) +
              geom_point(data = ith[ith$patient == pat.name,],aes(group,ith),colour="red") +
              theme_bw()
  
  sig_p3 <- ggplot() +
              geom_jitter(data = signature.table[signature.table$PATIENT_ID != pat.name,],aes(group,exposure,colour=group),
                          alpha=0.4,
                          position = position_dodge2(width = 0.4)) +
              scale_color_manual(values = colour_palettes$diagnosis_relapse) +
              geom_point(data = signature.table[signature.table$PATIENT_ID == pat.name,],aes(group,exposure),colour="red") +
              facet_wrap(. ~ signature,nrow = 1) +
              theme_bw() + theme(legend.position = "none",axis.text.x = element_text(angle = 90))
  
  pat_vig <- plot_grid(seg_plot,gene_p1,sig_p3,nrow=3,rel_heights = c(0.5,0.25,0.25),axis = "r",align = "h")
  return(pat_vig)
}

plotSegments <- function(object=NULL,pat.name=NULL,cn.max=15){
    diff_matrix_s <- object[,pat.name]
    #diff_matrix_s <- round(diff_matrix_s,digits = 0)
    fdfiltfull <- as.data.frame(str_split(string = names(diff_matrix_s),pattern = ":|-",simplify = TRUE))
    colnames(fdfiltfull) <- c("chromosome","start","end")
    ...
    chrom.len$flats <- getcoordinates(chr = chrom.len$Group.1,pos = chrom.len$x.max/2,chrom.len = chrom.len)
    chrom.len$flate <- getcoordinates(chr = chrom.len$Group.1,pos = chrom.len$x.max,chrom.len = chrom.len)
    stage <- unique(patient_data$tumour_stage_at_diagnosis[patient_data$britroc_number == gsub(pat.name,pattern = "BRITROC-",replacement = "")])
    substage <- as.character(unique(patient_data$tumour_substage_at_diagnosis[patient_data$britroc_number == gsub(pat.name,pattern = "BRITROC-",replacement = "")]))
    substage <- ifelse(is.na(substage),"",substage)
    
    title <- pat.name
    sub.title <- paste0("age: ",unique(patient_data$age[patient_data$britroc_number == gsub(pat.name,pattern = "BRITROC-",replacement = "")]),
                        " | ",
                        "stage: ",paste0(stage,substage),
                        " | ",
                        "platinum status: ",unique(patient_data$pt_sensitivity_at_reg[patient_data$britroc_number == gsub(pat.name,pattern = "BRITROC-",replacement = "")]),
                        " | ",
                        "prior lines: ",unique(patient_data$pre_reg_chemo[patient_data$britroc_number == gsub(pat.name,pattern = "BRITROC-",replacement = "")]))
    rect.col <- ifelse(seq_along(chrom.len$Group.1) %% 2 == 0,"white","grey95")
...
ggplot(norm_paired_changes_all) +
  geom_jitter(aes(status,change,group=status,color=status),position = position_dodge2(width = 0.3)) +
  #geom_boxplot(aes(status,change),outlier.alpha = 0) +
...
ggplot(norm_paired_changes_all) +
  geom_jitter(aes(as.factor(prior_lines),change,color=as.factor(prior_lines)),position = position_dodge2(width = 0.3)) +
  #geom_boxplot(aes(status,change),outlier.alpha = 0) +
  #geom_signif(aes(status,change),comparisons = list(c("resistant","sensitive"))) +
  facet_wrap(. ~ status) +
  theme_bw()

norm_paired_changes_sub$status <- patient_data$pt_sensitivity_at_reg[match(norm_paired_changes_sub$PATIENT_ID,
                                                                           paste0("BRITROC-",patient_data$britroc_number))]

norm_paired_changes_sub$prior_lines <- patient_data$pre_reg_chemo[match(norm_paired_changes_sub$PATIENT_ID,
                                                           paste0("BRITROC-",patient_data$britroc_number))]
ggplot(norm_paired_changes_sub) +
  geom_jitter(aes(status,change,group=status,color=status),position = position_dodge2(width = 0.3)) +
  #geom_boxplot(aes(status,change),outlier.alpha = 0) +
  geom_signif(aes(status,change),comparisons = list(c("resistant","sensitive"))) +
  scale_color_manual(values = colour_palettes$resist_sensitive) +
...
t1 <- norm_paired_changes_all %>%
  mutate(change_prop = ifelse(abs(change) > 3,"TRUE","FALSE")) %>%
  group_by(PATIENT_ID) %>%
  mutate(prop = sum(change_prop == "TRUE")/length(change_prop)) %>%
  dplyr::select(-gene,-arx,-rlps,-change,-chr,-change_prop) %>%
  distinct()

ggplot(t1) +
  geom_jitter(aes(status,prop,group=status,color=status),position = position_dodge2(width = 0.3)) +
  geom_boxplot(aes(status,prop),outlier.alpha = 0) +
  geom_signif(aes(status,prop),comparisons = list(c("resistant","sensitive")),test = "wilcox.test") +
  scale_color_manual(values = colour_palettes$resist_sensitive) +
  theme_bw()

t2 <- norm_paired_changes_sub %>%
  mutate(change_prop = ifelse(abs(change) > 3,"TRUE","FALSE")) %>%
  group_by(PATIENT_ID) %>%
  mutate(prop = sum(change_prop == "TRUE")/length(change_prop)) %>%
  dplyr::select(-gene,-arx,-rlps,-change,-chr,-change_prop) %>%
...
ggplot(norm_paired_changes_all) + geom_point(aes(arx,rlps,color=status),alpha=0.2) + theme_bw()
```

Patient-specific alterations, while difficult to associate with specific clinical events or features as a singular observation, did demonstrate biologically interesting genomic alterations. For example,  BRITROC-65 & BRITROC-37 demonstrated extreme gains of KRAS.Whereas other patietns had notable losses of clinically relevant or frequently altered genes such as BRITROC-36 which had a 16 copy loss of AKT2 and 6 copy loss of NF1, and BRITROC-242 & BRITROC-246 which demonstrated more than 10 copy loss of CCNE1.

```{r plot_vigs_1}
pat_vig_65 <- get_patient_vig(diff_matrix = diff_matrix,pat.name = "BRITROC-65")
ggsave2(pat_vig_65,filename = "plots/pat_vig_65.pdf",height = 12,width = 8)
ggsave2(pat_vig_65,filename = "plots/pat_vig_65.png",height = 12,width = 8,dpi = 300)
pat_vig_65
```

```{r plot_vigs_2}
pat_vig_37 <- get_patient_vig(diff_matrix = diff_matrix,pat.name = "BRITROC-37")
ggsave2(pat_vig_37,filename = "plots/pat_vig_37.pdf",height = 12,width = 8)
ggsave2(pat_vig_37,filename = "plots/pat_vig_37.png",height = 12,width = 8,dpi = 300)
pat_vig_37
```

```{r plot_vigs_3}
pat_vig_36 <- get_patient_vig(diff_matrix = diff_matrix,pat.name = "BRITROC-36")
ggsave2(pat_vig_36,filename = "plots/pat_vig_36.pdf",height = 12,width = 8)
ggsave2(pat_vig_36,filename = "plots/pat_vig_36.png",height = 12,width = 8,dpi = 300)
pat_vig_36
```

```{r plot_vigs_4}
pat_vig_242 <- get_patient_vig(diff_matrix = diff_matrix,pat.name = "BRITROC-242")
ggsave2(pat_vig_242,filename = "plots/pat_vig_242.pdf",height = 12,width = 8)
ggsave2(pat_vig_242,filename = "plots/pat_vig_242.png",height = 12,width = 8,dpi = 300)
pat_vig_242
```

```{r plot_vigs_5}
pat_vig_246 <- get_patient_vig(diff_matrix = diff_matrix,pat.name = "BRITROC-246")
ggsave2(pat_vig_246,filename = "plots/pat_vig_246.pdf",height = 12,width = 8)
ggsave2(pat_vig_246,filename = "plots/pat_vig_246.png",height = 12,width = 8,dpi = 300)
pat_vig_246
```
