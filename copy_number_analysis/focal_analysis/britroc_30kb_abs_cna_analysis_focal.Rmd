...
p2 <- p2 + labs(title = "diagnosis") + theme(legend.position = "none")
p3 <- p3 + labs(title = "relapse") + theme(legend.position = "none")
all_rates_leg <- get_legend(
  p2 + guides(fill = guide_legend(title = "type",nrow = 1)) +
...
all_rates <- plot_grid(plot_grid(p2 + labs(title = ""),p3 + labs(title = ""),labels = c("A","B")),all_rates_leg,nrow = 2,rel_heights = c(0.8,0.2))
...
```{r source_data_fig3B}
write.table(x = combined_rates,
            file = "../../source_data/figure_3B.tsv",
...
kable(cna_differences$amplification[order(cna_differences$amplification$p.value),]) %>%
  kable_styling() %>%
  scroll_box(height = "200px")

kable(cna_differences$deletion[order(cna_differences$deletion$p.value),]) %>%
  kable_styling() %>%
  scroll_box(height = "200px")
```
...
kable(cna_differences_paired$amplification[order(cna_differences_paired$amplification$p.value),]) %>%
  kable_styling() %>%
  scroll_box(height = "200px")

kable(cna_differences_paired$deletion[order(cna_differences_paired$deletion$p.value),]) %>%
  kable_styling() %>%
  scroll_box(height = "200px")
```
...
```{r source_data_S12A}
write.table(x = prior_rates[prior_rates$prior_lines %in% c("1","2","3"),],
            file = "../../source_data/figure_S12A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggsave2(filename = "plots/prior_paired_lines_cna_focal_paired_plot.png",
        plot = prior_paired_line_plot1,width = 8,height = 8,units = "in",dpi = 300)
ggsave2(filename = "plots/prior_paired_lines_cna_focal_paired_plot.pdf",
        plot = prior_paired_line_plot1,width = 8,height = 8,units = "in",dpi = 300)
prior_paired_line_plot1
```

```{r source_data_S12B}
write.table(x = prior_paired_rates,
            file = "../../source_data/figure_S12B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
```{r prior_plot_comb}
prior_lines_cna_focal_legend <- get_legend(
  prior_line_plot1 + 
    guides(fill = guide_legend(nrow = 1)) +
    theme(legend.position = "bottom")
)

prior_lines_cna_focal_plot <- plot_grid(prior_line_plot1,
                                        prior_paired_line_plot1,
                                        prior_lines_cna_focal_legend,
                                        nrow = 3,labels = c("A","B",""),
...
```{r source_data_S11}
write.table(x = paired_amp_changes[paired_amp_changes$PATIENT_ID %in% pats,],
            file = "../../source_data/figure_S11.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
```{r source_data_S13}
write.table(x = paired_amp_changes[paired_amp_changes$prior_lines %in% c(1,2),],
            file = "../../source_data/figure_S13.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
write.table(x = paired_amp_changes,
            file = "../../source_data/figure_S14.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
