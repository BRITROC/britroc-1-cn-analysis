...
#### Fishers exact for CNA rates - paired
...
kable(cna_differences$amplification) %>%
  kable_styling() %>%
  scroll_box(height = "200px")

kable(cna_differences$deletion) %>%
  kable_styling() %>%
  scroll_box(height = "200px")
```
...
kable(head(cna_copies_results_paired[cna_copies_results_paired$Gene %in% focal_genes_table$Gene.name,])) %>%
...
writeLines(text = as.character(cn_copies_pt_sensitive$Gene[cn_copies_pt_sensitive$qVal < 0.05]),sep = "\n",con = "data/gene_sensitive_GOlist.txt")
writeLines(text = as.character(cn_copies_pt_resist$Gene[cn_copies_pt_resist$qVal < 0.05]),sep = "\n",con = "data/gene_resist_GOlist.txt")
```

```{r pt_status_enrich_result}
if(file.exists("data/gene_sensitive_GOlist_analysis.txt")){
  tb <- read.table("data/gene_sensitive_GOlist_analysis.txt",header = T,sep = "\t",skip = 6)
  print("fishers")
  print("FDR correction")
  colnames(tb) <- c("GO.BioProcess","Ref.list","Test.list","expected","over.under","fold.enrichment","pval","qval")
  tb
}
```
