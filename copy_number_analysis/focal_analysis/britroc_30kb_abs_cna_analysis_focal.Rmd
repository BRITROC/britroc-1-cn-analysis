...
ggsave(plot = cp,filename = "plots/rlps_pt_rate_plot_CNA.pdf",width = 8,height = 6,units = "in",dpi = 300)
cp
```
...
pt_status_freq <- data.frame(freq=unlist(lapply(pt_samples,FUN=function(x) nrow(x)))) %>%
  rownames_to_column(var = "names") %>%
  mutate(pt_status = str_split(names,pattern = "\\.",simplify = T)[,1]) %>%
  mutate(group = str_split(names,pattern = "\\.",simplify = T)[,2])

pt_line_plot2 <- ggplot(pt_status_freq) +
  geom_col(aes(group,freq,fill=group)) +
  facet_wrap(. ~ pt_status) +
  scale_fill_manual(values = colour_palettes$diagnosis_relapse) +
  theme_bw() + theme(legend.position = "bottom")
...
pt_paired_lines_freq <- data.frame(freq=unlist(lapply(pt_paired_samples,FUN=function(x) nrow(x)))) %>%
  rownames_to_column(var = "names") %>%
  mutate(pt_paired_lines = str_split(names,pattern = "\\.",simplify = T)[,1]) %>%
  mutate(group = str_split(names,pattern = "\\.",simplify = T)[,2])

pt_paired_line_plot2 <- ggplot(pt_paired_lines_freq) +
  geom_col(aes(group,freq,fill=group)) +
  facet_wrap(. ~ pt_paired_lines) +
  scale_fill_manual(values = colour_palettes$diagnosis_relapse) +
  theme_bw() + theme(legend.position = "bottom")
...
kable(haed(cna_copies_results_paired[cna_copies_results_paired$Gene %in% focal_genes_table$Gene.name,])) %>%
...
kable(head(cna_copies_results[cna_copies_results$Gene %in% focal_genes_table$Gene.name,])) %>%
  kable_styling() %>% scroll_box(height = "200px")
```
...
paired_gene_change_prior_lines <- ggplot(paired_amp_changes[paired_amp_changes$prior_lines %in% c(1,2),]) +
  geom_point(aes(x = group,y = value,color=group),position = position_dodge2(width = 0.4)) +
  ...
  facet_wrap(as.factor(prior_lines) ~ gene,scales = "free") +
  theme_bw()

ggsave(plot = paired_gene_change_prior_lines,filename = "plots/paired_gene_change_prior_lines_plot.png",width = 6,height = 8,units = "in",dpi = 300)
ggsave(plot = paired_gene_change_prior_lines,filename = "plots/paired_gene_change_prior_lines_plot.pdf",width = 6,height = 8)
paired_gene_change_prior_lines
```

### Paired Pt status copies

```{r paired_pt_copies}
paired_gene_change_pt_status <- ggplot(paired_amp_changes) +
  geom_jitter(aes(x = group,y = value,color=group),position = position_dodge2(width = 0.4)) +
  geom_boxplot(aes(x = group,y = value,fill=group),alpha=0.3,position = "dodge") +
  geom_signif(aes(x = group,y = value),comparison=list(c("diagnosis","relapse")),vjust = 1.5,test = "wilcox.test") +
  facet_wrap(pt_status ~ gene,scales = "free") +
  theme_bw()

ggsave(plot = paired_gene_change_pt_status,filename = "plots/paired_gene_change_pt_status_plot.png",width = 6,height = 8,units = "in",dpi = 300)
ggsave(plot = paired_gene_change_pt_status,filename = "plots/paired_gene_change_pt_status_plot.pdf",width = 6,height = 8)
paired_gene_change_pt_status
```
...
cna_median_copies(data = norm_amp_changes_wide,min_obs = 3,
