---
title: "Combined copy number analysis - BriTROC"
author: "Philip Smith"
...
sig_bar$platinum_resistance <- factor(ifelse(sig_bar$PATIENT_ID %in% primary_resist_pats$PATIENT_ID,"primary resistant","non-primary resistant"),
...
patient.meta$platinum_resistance <- factor(ifelse(patient.meta$PATIENT_ID %in% primary_resist_pats$PATIENT_ID,
                                           "primary resistant","non-primary resistant"),
                                      levels = c("primary resistant","non-primary resistant"))
...
primary_pt_rates$platinum_resistance <- factor(primary_pt_rates$platinum_resistance,levels = c("primary resistant","non-primary resistant"))
...
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant"],
                     genes = focal_genes_table$Gene.name)
```
...
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$platinum_resistance == "primary resistant"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$platinum_resistance == "non-primary resistant"],
                     genes = focal_genes_table$Gene.name)
```
...
paired_amp_changes_all$platinum_resistance <- factor(paired_amp_changes_all$platinum_resistance,levels = c("primary resistant","non-primary resistant"))
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant"],
                         p.adj = "fdr")
```

```{r copies_primary_resistance_2}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant" & patient.meta$group == "diagnosis"],
                         p.adj = "fdr")
```

```{r copies_primary_resistance_3}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant" & patient.meta$group == "relapse"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```

```{r copies_primary_resistance_4}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "primary resistant" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$platinum_resistance == "non-primary resistant" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```
...
primary_pt_copies_plot1 <- ggplot(paired_amp_changes[paired_amp_changes$group == "diagnosis",]) +
  ...
  #geom_signif(aes(x = platinum_resistance,y = value),comparison=list(c("primary resistant","non-primary resistant")),
...
norm_amp_changes$platinum_resistant <- factor(ifelse(norm_amp_changes$PATIENT_ID %in% primary_resist_pats$PATIENT_ID,
                                           "primary resistant","non-primary resistant"),levels = c("primary resistant","non-primary resistant"))
...
primary_pt_change_plot1 <- ggplot(norm_amp_changes) +
  ...
  #geom_signif(aes(x = platinum_resistant,y = change),comparison=list(c("primary resistant","non-primary resistant")),test = "wilcox.test",vjust = 1.5) +
  facet_wrap(. ~ gene,scales = "free_y",ncol = 3) +
  theme_bw() +
  theme(legend.position = "none")
