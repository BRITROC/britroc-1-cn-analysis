...
brca_paired_amp_changes_plot <- ggplot(paired_amp_changes) +
    ...
    scale_fill_manual(values = colour_palettes$brca_carriers) +
    scale_colour_manual(values = colour_palettes$brca_carriers) +
    geom_signif(aes(xmin=xmin,xmax=xmax,y_position=unlist(lapply(split(paired_amp_changes,f = factor(paired_amp_changes$gene)),FUN = function(x) round(max(x$value))+2)),annotations=round(qVal,digits = 3),group=gene),
