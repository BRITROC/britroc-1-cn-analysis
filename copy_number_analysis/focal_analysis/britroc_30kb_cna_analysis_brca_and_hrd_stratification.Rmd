...
sig_bar$BRCA_carrier <- ifelse(sig_bar$PATIENT_ID %in% combinedBRCA$PATIENT_ID,"BRCA-mutant","wildtype")
...
patient.meta$BRCA_carrier <- factor(ifelse(patient.meta$PATIENT_ID %in% combinedBRCA$PATIENT_ID,
                                           "BRCA-mutant","wildtype"),levels = c("BRCA-mutant","wildtype"))
...
brca_rates$BRCA_carrier <- factor(brca_rates$BRCA_carrier,
                                  levels = c("BRCA-mutant","wildtype"))
```

```{r cna_rate_brca_non_brca_1}
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype"],
                     genes = focal_genes_table$Gene.name)
```
...
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$BRCA_carrier == "BRCA-mutant"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$BRCA_carrier == "wildtype"],
                     genes = focal_genes_table$Gene.name)
```
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "diagnosis"],
                         p.adj = "fdr")
```

```{r copies_brca_non_brca_3}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "relapse"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```

```{r copies_brca_non_brca_5}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "diagnosis"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "relapse"],
                         p.adj = "fdr")
```
...
ggplot(amp_changes) +
    ...
    geom_signif(aes(x = BRCA_carrier,y = value),comparison=list(c("BRCA-mutant","wildtype")),
...
brca_sig_dist_plot <- ggplot(data = sig_bar) +
    ...
    geom_signif(aes(x = BRCA_carrier,y = value),
                test = "wilcox.test",comparisons = list(c("BRCA-mutant","wildtype")),
                vjust = 1.8) +
    scale_fill_manual(values = colour_palettes$brca_carriers) +
    scale_colour_manual(values = colour_palettes$brca_carriers) +
    ylab("exposure") +
...
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$paired == "TRUE"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$paired == "TRUE"],
                     genes = focal_genes_table$Gene.name)
```
...
fishers_gene_CNA(data = cna_calls,
                     sub_a = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$paired == "TRUE"],
                     sub_b = patient.meta$SAMPLE_ID[patient.meta$group == "diagnosis" &
                                                         patient.meta$BRCA_carrier == "wildtype" & patient.meta$paired == "TRUE"],
                     genes = focal_genes_table$Gene.name)
```
...
brca_diag_rlps_cna_rates_paired_plot <- ggplot(brca_rates_paired[brca_rates_paired$group == "amplification",]) +
  geom_col(aes(x = gene,y = rate,fill=tumour),position = "dodge") +
  scale_fill_manual(values = colour_palettes$diagnosis_relapse) +
  ylab(label = "rate (%)") +
  facet_grid(cols = vars(BRCA_carrier),scales = "free_x") +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(vjust = 0.5,hjust = 1,angle = 90),
        legend.position = "none")
...
brca_cna_rates_paired_plot_comb <- plot_grid(brca_diag_rlps_cna_rates_paired_plot,
                                             brca_cna_rates_paired_plot,
          plot_grid(brca_cna_rates_paired_diag_plot,
                    brca_cna_rates_paired_relapse_plot,ncol = 2,labels = c("B","C")),
          brca_cna_rates_paired_legend,labels = c("A","B","",""),
          nrow = 4,
          rel_heights = c(0.6,0.3,0.1))

ggsave(plot = brca_cna_rates_paired_plot,
       filename = "plots/brca_cna_rates_paired_plot.png",width = 6,height = 8,units = "in",dpi = 300)
ggsave(plot = brca_cna_rates_paired_plot_comb,
       filename = "plots/brca_cna_rates_paired_plot.pdf",width = 6,height = 8)
saveRDS(object = brca_cna_rates_paired_plot,file = "plots/brca_cna_rates_paired_plot.rds")
brca_cna_rates_paired_plot

ggsave(plot = brca_cna_rates_paired_plot_comb,
       filename = "plots/brca_diag_rlps_cna_rates_paired_plot.png",width = 6,height = 8,units = "in",dpi = 300)
ggsave(plot = brca_cna_rates_paired_plot_comb,
       filename = "plots/brca_diag_rlps_cna_rates_paired_plot.pdf",width = 6,height = 8)
saveRDS(object = brca_diag_rlps_cna_rates_paired_plot,file = "plots/brca_diag_rlps_cna_rates_paired_plot.rds")
brca_diag_rlps_cna_rates_paired_plot
```

```{r source_data_8A}
write.table(x = brca_rates_paired[brca_rates_paired$group == "amplification",],
            file = "../../source_data/figure_8A.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```

```{r source_data_8B}
write.table(x = brca_rates_paired,
            file = "../../source_data/figure_8B.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "diagnosis" & patient.meta$paired == "TRUE"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "diagnosis" & patient.meta$paired == "TRUE"],
                         p.adj = "fdr")
```

```{r copies_brca_non_brca_paired_3}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "relapse" & patient.meta$paired == "TRUE"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "relapse" & patient.meta$paired == "TRUE"],
                         p.adj = "fdr")
```
...
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "diagnosis" & patient.meta$paired == "TRUE"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$group == "relapse" & patient.meta$paired == "TRUE"],
                         p.adj = "fdr")
```

```{r copies_brca_non_brca_paired_5}
cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "diagnosis" & patient.meta$paired == "TRUE"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$group == "relapse" & patient.meta$paired == "TRUE"],
                         p.adj = "fdr")
```
...
sigAnn <- cna_median_copies(data = extracted,min_obs = 3,genes = focal_genes_table$Gene.name,
                         sub_a = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "BRCA-mutant" & patient.meta$paired == "TRUE"],
                         sub_b = patient.meta$SAMPLE_ID[patient.meta$BRCA_carrier == "wildtype" & patient.meta$paired == "TRUE"],
                         p.adj = "fdr") %>%
    arrange(Gene) %>%
    mutate(xmin = rep(1,nrow(.))) %>%
    mutate(xmax = rep(2,nrow(.))) %>%
    mutate(y_position = rep(1,nrow(.))) %>%
    rename("gene"="Gene")

brca_paired_amp_changes_plot <- ggplot(paired_amp_changes) +
    geom_point(aes(x = BRCA_carrier,y = value,color=BRCA_carrier),position = position_dodge2(width = 0.4)) +
    geom_violin(aes(x = BRCA_carrier,y = value,fill=BRCA_carrier),alpha=0.3,position = "dodge") +
    scale_fill_manual(values = colour_palettes$brca_carriers) +
    scale_colour_manual(values = colour_palettes$brca_carriers) +
    geom_signif(aes(xmin=xmin,xmax=xmax,
                    y_position=c(12,42,10,6,7,10,10,7,41,12,12,29,15,14,10,13,8,6),
                    annotations=round(qVal,digits = 3),group=gene),
                data = sigAnn,manual = TRUE,vjust = 1.5) +
    facet_wrap(. ~ gene,scales = "free_y",ncol = 3) +
    theme_bw() +
    theme(legend.position = "bottom")

brca_paired_amp_changes_strat_plot <- ggplot(paired_amp_changes) +
    geom_point(aes(x = BRCA_carrier,y = value,color=BRCA_carrier),position = position_dodge2(width = 0.4)) +
    geom_violin(aes(x = BRCA_carrier,y = value,fill=BRCA_carrier),alpha=0.3,position = "dodge") +
    scale_fill_manual(values = colour_palettes$brca_carriers) +
    scale_colour_manual(values = colour_palettes$brca_carriers) +
...
ggsave(plot = brca_paired_amp_changes_plot,filename = "plots/brca_paired_amp_changes_plot.png",width = 6,height = 8,units = "in",dpi = 300)
ggsave(plot = brca_paired_amp_changes_plot,filename = "plots/brca_paired_amp_changes_plot.pdf",width = 6,height = 8)
saveRDS(object = brca_paired_amp_changes_plot,file = "plots/brca_paired_amp_changes_plot.rds")
brca_paired_amp_changes_plot
```

```{r source_data_8C}
write.table(x = paired_amp_changes,
            file = "../../source_data/figure_8C.tsv",
            quote = F,sep = "\t",row.names = F,col.names = T)
```
...
ggplot(norm_amp_changes) +
    ...
    scale_colour_manual(values = colour_palettes$brca_carriers) +
    geom_signif(aes(x = BRCA_carrier,y = change),
        comparison=list(c("BRCA-mutant","wildtype")),
        test = "wilcox.test",vjust = 1.5) +
    facet_wrap(. ~ gene,scales = "free_y",ncol = 3) +
    theme_bw() +
    theme(legend.position = "bottom")
...
seg_paired_corr_plot <- ggplot(seg_paired_corr) +
                        ...
                        scale_colour_manual(values = colour_palettes$brca_carriers) +
                        xlab("diagnosis segments") +
                        ylab("relapse segments") +
                        coord_equal() +
                        theme_bw()
