...
suppressPackageStartupMessages(suppressWarnings(library(compositions)))
...
saveRDS(object = brca_cna_rates_plot,file = "plots/brca_cna_rates_plot.rds")
...
saveRDS(object = brca_sig_bar_plot,file = "plots/brca_sig_bar_plot.rds")
...
saveRDS(object = brca_sig_dist_grouped_plot,file = "plots/brca_sig_dist_grouped_plot.rds")
...
```{r pca_sigs}
sigPca_noTrans <- prcomp(t(sig_quants),scale. = TRUE,center = TRUE)

sigPca_noTransDplyr <- as.data.frame(sigPca_noTrans$x) %>%
                        rownames_to_column(var = "SAMPLE_ID") %>%
                        mutate('BRCA status' = sig_bar$BRCA_carrier[match(SAMPLE_ID,sig_bar$SAMPLE_ID)])

ggplot(sigPca_noTransDplyr) +
  geom_point(aes(PC1,PC2,colour=`BRCA status`)) +
  scale_colour_manual(values = colour_palettes$brca_carriers) +
  theme_bw()

ggplot(sigPca_noTransDplyr) +
  geom_point(aes(PC2,PC3,colour=`BRCA status`)) +
  scale_colour_manual(values = colour_palettes$brca_carriers) +
  theme_bw()

ggplot(sigPca_noTransDplyr) +
  geom_point(aes(PC3,PC4,colour=`BRCA status`)) +
  scale_colour_manual(values = colour_palettes$brca_carriers) +
  theme_bw()
```

```{r pca_sigs_ALR}
sig_quants_ALR <- sig_quants
# Impute zeros
sig_quants_ALR[sig_quants_ALR == 0] <- 0.001
# ALR transformation
sig_quants_ALR <- alr(t(sig_quants_ALR))

sigPca_ALR <- prcomp(sig_quants_ALR,scale. = TRUE,center = TRUE)

sigPca_ALRDplyr <- as.data.frame(sigPca_ALR$x) %>%
                        rownames_to_column(var = "SAMPLE_ID") %>%
                        mutate('BRCA status' = sig_bar$BRCA_carrier[match(SAMPLE_ID,sig_bar$SAMPLE_ID)])

ggplot(sigPca_ALRDplyr) +
  geom_point(aes(PC1,PC2,colour=`BRCA status`)) +
  scale_colour_manual(values = colour_palettes$brca_carriers) +
  theme_bw()
...
```{r pca_sigs_ILR}
sig_quants_ILR <- sig_quants
# Impute zeros
sig_quants_ILR[sig_quants_ILR == 0] <- 0.001
# ILR transformation
sig_quants_ILR <- ilr(t(sig_quants_ILR))

sigPca_ILR <- prcomp(sig_quants_ILR,scale. = TRUE,center = TRUE)

sigPca_ILRDplyr <- as.data.frame(sigPca_ILR$x) %>%
                        rownames_to_column(var = "SAMPLE_ID") %>%
                        mutate('BRCA status' = sig_bar$BRCA_carrier[match(SAMPLE_ID,sig_bar$SAMPLE_ID)])

ggplot(sigPca_ILRDplyr) +
  geom_point(aes(PC1,PC2,colour=`BRCA status`)) +
  scale_colour_manual(values = colour_palettes$brca_carriers) +
  theme_bw()
...
```{r events_plot}
brca_cn_event_comb_plot <- plot_grid(cn_event_plots,paired_cn_event_plots,
          plot_grid(brca_diag_relapse_leg,brca_cna_rates_legend),
          rel_heights = c(0.45,0.45,0.1),
          nrow = 3,
          labels = c("A","B",""))

ggsave(plot = brca_cn_event_comb_plot,filename = "plots/brca_cn_event_comb_plot.png",width = 6,height = 6,units = "in",dpi = 300)
ggsave(plot = brca_cn_event_comb_plot,filename = "plots/brca_cn_event_comb_plot.pdf",width = 6,height = 6)
